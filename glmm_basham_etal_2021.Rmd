...
## Prepare packages
devtools::source_url("https://raw.githubusercontent.com/slamander/load_packages/main/prepare_libraries.R")
...
global_dat <- survey_tree_dat %>%
  ...
  relocate(tree_code, Season, date, am_pm, arboreal_survey, ground_survey)
...
pres_mods_formulas <-  c(
  "pres ~ 1 +                  geography + (1|tree_code)", 
  "pres ~ epiphytes +                  geography + (1|tree_code)", 
  "pres ~ mean_canopy_cover +          geography + (1|tree_code)", 
  "pres ~ mean_ground_cover +          geography + (1|tree_code)",
  "pres ~ canopy_height +              geography + (1|tree_code)",
  "pres ~ large_vines +                geography + (1|tree_code)",
  "pres ~ moss +                       geography + (1|tree_code)",
  "pres ~ dbh +                        geography + (1|tree_code)",
  "pres ~ root_base +                  geography + (1|tree_code)",
  "pres ~ lugovia_epiphytes +          geography + (1|tree_code)",
  "pres ~ stream_distance +            geography + (1|tree_code)",
  "pres ~ density_small_stems +        geography + (1|tree_code)",
  "pres ~ density_big_stems +          geography + (1|tree_code)",
  "pres ~ espave +                     geography + (1|tree_code)",
  "pres ~ mean_canopy_cover*espave +   geography + (1|tree_code)",
  "pres ~ mean_ground_cover*espave +   geography + (1|tree_code)",
  "pres ~ canopy_height*espave +       geography + (1|tree_code)",
  "pres ~ large_vines*espave +         geography + (1|tree_code)",
  "pres ~ moss*espave +                geography + (1|tree_code)",
  "pres ~ dbh*espave +                 geography + (1|tree_code)",
  "pres ~ epiphytes*espave +           geography + (1|tree_code)",
  "pres ~ root_base*espave +           geography + (1|tree_code)",
  "pres ~ lugovia_epiphytes*espave +   geography + (1|tree_code)",
  "pres ~ stream_distance*espave +     geography + (1|tree_code)")
...
pres_marg_R2 <- lapply(pres_mods, function(x){
  r.squaredGLMM(x)[2,1]}) %>% 
  unlist() 
pres_cond_R2 <- lapply(pres_mods, function(x){
  r.squaredGLMM(x)[2,2]}) %>% 
  unlist() 

pres_mods_output$marg_R2 <- pres_marg_R2
pres_mods_output$cond_R2 <- pres_cond_R2
...
global_dat_pres_dbh_espave <- cbind(preds = glmer(as.formula(pres_mods_output$Modnames[2]), family = binomial, data = global_dat) %>% 
  predict(type = "response"), global_dat)
global_dat_pres_stream_espave <- cbind(preds = glmer(as.formula(pres_mods_output$Modnames[3]), family = binomial, data = global_dat) %>% 
  predict(type = "response"), global_dat)
...
gg_pres_dbh <- ggplot(data = global_dat_pres_dbh, aes(x = dbh, y = preds, col = "All")) + 
  ...
  coord_cartesian(ylim = c(0,1)) + theme_classic() + theme(legend.position = c(.8,.2))

gg_pres_dbh_espave <- ggplot(data = global_dat_pres_dbh_espave, aes(x = dbh, y = preds, col = espave_labels)) + 
  stat_smooth(size = 1, alpha = 0.5, method = "glm", method.args = list(family = "quasibinomial")) +
  xlab("Diameter at Breast Height (scaled)") + ylab("Probability of Presence") +
  scale_color_manual("Tree Species", values = c("orange","grey25"), labels = c("Non-Espavé","Espavé")) + ylim(c(-0.1,1.0)) +
  theme_classic() + theme(legend.position = c(.8,.25))

gg_pres_stream_espave <- ggplot(data = global_dat_pres_stream_espave, aes(x = stream_distance, y = preds, col = espave_labels)) + 
  stat_smooth(size = 1, alpha = 0.5, method = "glm", method.args = list(family = "quasibinomial")) +
  xlab("Distance to stream (scaled)") + ylab("Probability of Presence") + ylim(0,1) +
  scale_color_manual("Tree Species", values = c("orange","grey25"), labels = c("Non-Espavé","Espavé")) + 
  theme_classic() + theme(legend.position = "none")

pres_3_pan <- ggarrange(gg_pres_dbh, gg_pres_dbh_espave, gg_pres_stream_espave,
          ncol = 1, labels = c("(a)","(b)","(c)"),
          label.x = 0.12, label.y = 0.99)
```
...
# Prepare data for presence and counts modeling
```{r Count models, message=FALSE, warning=FALSE}
# count_pseudor2 <- r.squaredGLMM(glmmTMB(count ~ 1 + (1|tree_code), ziformula=~0, family = nbinom2, data = global_dat))           
# count_pseudor2_geo <- r.squaredGLMM(glmmTMB(count ~ 1 + geography + (1|tree_code), ziformula=~0, family = nbinom2, data = global_dat))  
# count_pseudor2 # delta: cond = 0.7351654, marg = 0
# count_pseudor2_geo # delta: cond = 0.7622064, marg = 0.9995989
# 
# 0.7622064 - 0.7351654 # difference in delta; 0.0098095
```

```{r}
count_mods_formulas <- c(
  "count ~ 1 +                        geography + (1|tree_code)", 
  "count ~ epiphytes +                geography + (1|tree_code)", 
  "count ~ mean_canopy_cover +        geography + (1|tree_code)", 
  "count ~ mean_ground_cover +        geography + (1|tree_code)",
  "count ~ canopy_height +            geography + (1|tree_code)",
  "count ~ large_vines +              geography + (1|tree_code)",
  "count ~ moss +                     geography + (1|tree_code)",
  "count ~ dbh +                      geography + (1|tree_code)",
  "count ~ root_base +                geography + (1|tree_code)",
  "count ~ lugovia_epiphytes +        geography + (1|tree_code)",
  "count ~ stream_distance +          geography + (1|tree_code)",
  "count ~ density_small_stems +      geography + (1|tree_code)",
  "count ~ density_big_stems +        geography + (1|tree_code)",
  "count ~ espave +                   geography + (1|tree_code)",
  "count ~ mean_canopy_cover*espave + geography + (1|tree_code)",
  "count ~ mean_ground_cover*espave + geography + (1|tree_code)",
  "count ~ canopy_height*espave +     geography + (1|tree_code)",
  "count ~ large_vines*espave +       geography + (1|tree_code)", 
  "count ~ moss*espave +              geography + (1|tree_code)",
  "count ~ dbh*espave +               geography + (1|tree_code)",
  "count ~ stream_distance*espave +   geography + (1|tree_code)")
...
count_marg_R2 <- lapply(count_mods, function(x){
  r.squaredGLMM(x)[2,1]}) %>% 
  unlist() 
count_cond_R2 <- lapply(count_mods, function(x){
  r.squaredGLMM(x)[2,2]}) %>% 
  unlist() 

count_mods_output$marg_R2 <- count_marg_R2
count_mods_output$cond_R2 <- count_cond_R2

```

```{r}
global_dat_count_stream_espave <- cbind(preds = glmmTMB(as.formula(count_mods_output$Modnames[1]), ziformula=~0, family = nbinom2, data = global_dat) %>% 
  predict(type = "response"), global_dat)
global_dat_count_espave <- cbind(preds = glmmTMB(as.formula(count_mods_output$Modnames[2]), ziformula=~0, family = nbinom2, data = global_dat) %>% 
  predict(type = "response"), global_dat)
global_dat_count_dbh_espave <- cbind(preds = glmmTMB(as.formula(count_mods_output$Modnames[3]), ziformula=~0, family = nbinom2, data = global_dat) %>% 
  predict(type = "response"), global_dat)

gg_count_stream_espave <- ggplot(data = global_dat_count_stream_espave, aes(x = stream_distance, y = preds, col = espave_labels)) + 
  stat_smooth(size = 1, alpha = 0.5, method = "glm", method.args = list(family = "quasipoisson")) +
  xlab("Distance to Stream (scaled)") + ylab("Predicted Abundance") + coord_cartesian(ylim = c(0,8)) +
  scale_color_manual("Tree Species", values = c("orange","grey25"), labels = c("Non-Espavé","Espavé")) + 
  theme_classic() + theme(legend.position = "none")

gg_count_espave <- ggplot(global_dat_count_espave, aes(x = espave, y = preds, fill = espave_labels)) + 
  geom_boxplot(alpha = 0.7, width = 0.5, size = 0.75) + xlab(" ") + ylab("Predicted Abundance") + coord_cartesian(ylim = c(0,8)) +
...
gg_count_dbh_espave <- ggplot(global_dat_count_dbh_espave, aes(x = dbh, y = preds, col = espave_labels)) + 
  stat_smooth(size = 1, alpha = 0.5, method = "glm", method.args = list(family = "quasipoisson")) +
  xlab("Diameter at Breast Height (scaled)") + ylab("Predicted Abundance") + coord_cartesian(ylim = c(0,8)) +
  scale_color_manual("Tree Species", values = c("orange","grey25"), labels = c("Non-Espavé","Espavé")) + 
  theme_classic() + theme(legend.position = "none")

ggarrange(gg_count_stream_espave, gg_count_espave, gg_count_dbh_espave,
          ncol = 1, labels = c("(a)","(b)","(c)"),
          label.x = 0.08, label.y = 0.99); ggsave("count_3panel.png", width = 4, height = 8, dpi = 600)
```
...
count_3_pan <- ggarrange(gg_count_stream_espave, gg_count_espave, gg_count_dbh_espave,
          ncol = 1, labels = c("(d)","(e)","(f)"),
          label.x = 0.08, label.y = 0.99)
